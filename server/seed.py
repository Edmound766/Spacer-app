...
def seed_availability_test_data():
    ...
    user1 = User(username="john_doe", email="john@example.com",role=admin_role)
    user1.password=fake.password()

    user2 = User(username="jane_doe", email="jane@example.com",role=client_role)
    ...
    booking1 = Booking(
        created_at=datetime.utcnow() - timedelta(hours=1),  # created 1 hour ago
        duration_hours=2,
        name=fake.name(),
        user_id=user2.id,
        space_id=space1.id
    ...
    booking2 = Booking(
        created_at=datetime.utcnow() - timedelta(minutes=30),  # created 30 minutes ago
        duration_hours=1,
        name=fake.name(),
        user_id=user1.id,
        space_id=space1.id
    ...
    booking3 = Booking(
        created_at=datetime.utcnow() - timedelta(hours=3),  # created 3 hours ago
        duration_hours=2,
        name=fake.name(),
        user_id=user1.id,
        space_id=space2.id
